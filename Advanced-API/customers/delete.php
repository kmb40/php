<?php
# This api deletes data from the database
...
header('Access-Control-Allow-Method: DELETE'); // Only responds to GET request
...
$requestMethod = $_SERVER["REQUEST_METHOD"]; // Determines requestors request type (post,get,etc)
    ...
    if($requestMethod == "DELETE") { // If request if of type GET

            $deleteCustomer = deleteCustomer($_GET); // Set variable customer to function getCustomer
            echo $deleteCustomer; // Display customer
